...
import com.rmn.toolkit.credits.query.util.ClientUtil;
...
public class SecurityUtil {
    ...
    public void checkPermissionToViewCredits(String clientId, String authorizationHeader) {
        String clientIdFromToken = getCurrentUserId();
        String roleName = getCurrentUserRole(authorizationHeader);
        if (!Objects.equals(clientId, clientIdFromToken) && !ADMIN.equals(roleName)) {
            log.error("Client hasn't got enough rights to view credits");
            throw new AccessDeniedException("Client hasn't got enough rights to view credits");
        }
    }
